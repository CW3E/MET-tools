...
if len(FCST_STRT) != 10:
    ...
    fcst_strt = dt.fromisoformat(FCST_STRT[:4] + '-' + FCST_STRT[4:6] + '-' +
            FCST_STRT[6:8] + '_' + FCST_STRT[8:])

if len(FCST_END) != 10:
    print('ERROR: FCST_END, ' + FCST_END + ', is not in YYYYMMDDHH format.')
    sys.exit(1)
else:
    fcst_end = dt.fromisoformat(FCST_END[:4] + '-' + FCST_END[4:6] + '-' +
            FCST_END[6:8] + '_' + FCST_END[8:])
